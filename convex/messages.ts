...
export const get = query({
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      page: (
        await Promise.all(
          results.page.map(async (message: Doc<"messages">) => {
            ...
            const image: string | null | undefined = message.image
              ? await ctx.storage.getUrl(message.image)
              : undefined;
            const reactionsWithCount = reactions.map(
              (reaction, index, thisArray) => ({
                ...reaction,
                count: thisArray.filter((r) => r.value === reaction.value)
                  .length,
              })
            );
